...
const poolDeposit = async () => {
  ...
  try {
    ...
    const [pendingWallets] = await conn.execute('SELECT paymentid FROM pending_wallets');
    ...
    for (const w of pendingWallets) {
      try {
        const response = await axios.get(
          ...
          {
            headers: { "x-api-key": process.env.NOWPAYAPIKEY },
            timeout: 15000
          }
        );
        ...
        if (existing.length) {
          await conn.execute('DELETE FROM pending_wallets WHERE paymentid=?', [providerPaymentId]);
          continue;
        }
        ...
        await conn.execute('UPDATE deposits SET credited=1 WHERE id=?', [result.insertId]);
        await conn.execute('DELETE FROM pending_wallets WHERE paymentid=?', [providerPaymentId]);
        await conn.commit();
...
let Withdraw = async (req, res) => {
  ...
  try {
    ...
    if (!addressRegex.test(withdrawaddress)) {
      return res.status(400).json({ message: "Address should not contain special characters" });
    }
    ...
    return res.status(200).json({ message: "Withdraw Initiated, will be processed soon" });
  ...
  try {
    ...
    for (const req of pending) {
      try {
        ...
        const code2fa = speakeasy.totp({ secret: TOTP_SECRET, encoding: 'base32' });
...
const ResetEmail = async (req, res) => {
  ...
  try {
    ...
    const [user] = await connection.execute(`SELECT userid FROM users WHERE email = ?`, [email.trim()]);
    if (user.length === 0) return res.status(403).json({ message: "Invalid email!" });

    const uid = user[0].userid;

    const [rows] = await connection.query(
      `SELECT COUNT(*) AS count FROM resetpassword WHERE userid = ? AND expire_at > NOW()`,
      [uid]
    );

    if (rows[0].count >= 3) return res.status(409).json({ message: "Maximum reset requests reached. Try again later." });

    const token = crypto.randomBytes(32).toString("hex");
    const hashedToken = crypto.createHash("sha256").update(token).digest("hex");

    await connection.query(
      `INSERT INTO resetpassword(userid, email, token, expire_at) VALUES (?, ?, ?, NOW() + INTERVAL 15 MINUTE)`,
    ...
    if (emailStatus.success) return res.status(200).json({ message: "Password reset email sent" });
...
const ReceivetokenANDReset = async (req, res) => {
  ...
  try {
    ...
    if (!token || !password) return res.status(400).json({ message: "Password are required" });
    if (password.length < 6) return res.status(400).json({ message: "Password must be at least 6 characters" });

    connection = await con.getConnection();

    const [rows] = await connection.execute(
      `SELECT userid FROM resetpassword WHERE token = ? AND used = 0 AND expire_at > NOW() LIMIT 1`,
      [token]
    );
...
module.exports = {
  ...
  processWithdraw,
  ResetEmail,
  ReceivetokenANDReset,
  truncateResetPasswordTable
};